...
export function WatchCenter({ companies, findings, runs, onStatusChange, onOpenCompany }: WatchCenterProps) {
  ...
  const watchEnabledById = useMemo(
    () => new Map(companies.map((view) => [view.company.id, view.company.watchEnabled])),
    [companies],
  )
  const watchCompanies = useMemo(
    () => companies.filter((view) => view.company.watchEnabled),
    [companies],
  )
  const enabledFindings = useMemo(
    () => findings.filter((item) => watchEnabledById.get(item.userCompanyId) !== false),
    [findings, watchEnabledById],
  )
  const visible = useMemo(() => [...enabledFindings].filter((item) => {
    if (status === 'all' && item.status === 'dismissed') return false
  ...
  }), [companyId, enabledFindings, severity, status])
  ...
  return (
    <section className="page-stack" aria-labelledby="watch-title">
      ...
      <div className="metric-grid">
        ...
        <article className="metric-card"><span>要対応</span><strong>{enabledFindings.filter((item) => ['new', 'seen'].includes(item.status)).length}</strong><small>new + seen</small></article>
        <article className="metric-card"><span>完了</span><strong>{enabledFindings.filter((item) => item.status === 'completed').length}</strong><small>再取込でもnewへ戻しません</small></article>
      ...
      <div className="watch-filters">
        ...
        <label className="field"><span>企業</span><select value={companyId} onChange={(event) => setCompanyId(event.target.value)}><option value="all">すべて</option>{watchCompanies.map((view) => <option key={view.company.id} value={view.company.id}>{view.displayName}</option>)}</select></label>
      </div>
        ...
        <div className="watch-list">
          {visible.map((finding) => (
            <article className={`watch-card severity-${finding.severity}`} key={finding.id}>
              ...
              {finding.source && <details><summary>根拠を見る</summary><p>{finding.source.title} / {finding.source.type}</p>{finding.source.url && isSafeHttpUrl(finding.source.url) && <a href={finding.source.url} target="_blank" rel="noreferrer">出典を開く ↗</a>}</details>}
