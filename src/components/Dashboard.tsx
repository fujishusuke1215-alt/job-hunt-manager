          ...
          <article className="panel deadline-panel">
            ...
            <div className="deadline-list">
              {actions.length === 0 ? <p className="muted-message">未完了の予定・Watchはありません。</p> : actionGroups.map(([label, group]) => group.length > 0 && <div className="action-group" key={label}><h3>{label}</h3>{group.map((item) => (
                <div className="deadline-row-wrap todo-row-wrap" key={item.id}><div className="todo-row-main"><button className="todo-complete-button" type="button" aria-label={`${item.companyName} ${item.title}を完了にする`} onClick={() => complete(item)}>✓</button><button className="deadline-row" type="button" onClick={() => onOpenAction ? onOpenAction(item) : item.source === 'watch_finding' ? onOpenWatch() : onOpenCompany(item.userCompanyId)}>
