...
## Phase 20: 一般向けUXと将来の匿名統計の準備

- 通常UIは企業マスタ、恒久ID、A/B/C優先度を露出せず、企業名とURLから安全な内部候補照合を行う。
- 選考は段階・終了状態・終了理由・内定後判断を分離し、不合格と選考管理終了を区別する。
- 新規・公開デモは10点満点の「バランス型」評価を使う。旧評価設定は既存利用者向けに残す。
- 個人データだけに最終利用時刻、選考段階更新時刻、任意の卒業年を保存する。共有・匿名統計・Gmail取得は今回実装しない。
