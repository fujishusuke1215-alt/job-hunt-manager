...
function toFinding_(message, now) {
  ...
  const due=/deadline|required|reservation|document/i.test(actionType||'')&&parsed?(parsed.at||new Date(`${parsed.date}T23:59:00+09:00`).toISOString()):null, starts=/scheduled|invitation/i.test(actionType||'')&&parsed?.at?parsed.at:null, sender=heads.from||'', rfcMessageId=String(heads['message-id']||'').trim(), account=requiredExpectedGmailAccount_(), fallback=`from:(${sender.match(/<([^>]+)>/)?.[1]||sender}) subject:(${subject.replace(/"/g,'').slice(0,120)})`, query=rfcMessageId?`rfc822msgid:${rfcMessageId.replace(/^<|>$/g,'')}`:fallback, target=`https://mail.google.com/mail/u/0/#search/${encodeURIComponent(query)}`, search=`https://accounts.google.com/AccountChooser?Email=${encodeURIComponent(account)}&continue=${encodeURIComponent(target)}`, myPageUrl=urls.find(url=>/^https:\/\//i.test(url)&&/mypage|my-page|entry|recruit|career/i.test(url)&&/マイページ|MyPage|ログイン|採用/i.test(all))||null;
